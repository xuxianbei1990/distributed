...
public class MyHashMap<K, V> {
    ...
    /**
     * 因为 p = tab[i = (n - 1) & hash] 从数组中获取key是通过hash & 数组的长度 来获取的。
     * 而数组的长度一般小于2的16次。 一般只有低位的Hash参与，如果低位hash一样高位不一样的话，那么就会产生碰撞
     * 所以这里通过^ 让高位也参与hash计算。减少碰撞。
     * @param key
     * @return
     */
                ...
                if ((e = oldTab[j]) != null) {
                    ...
                    if (e.next == null) {
                        ...
                        do {
                             ...
                             * 不需要重新计算hash，只需要根据原来hash值和原始数组进行与运算的判断1还是0分别放进两个链表lo和hi（非红黑树的情况）里，
                             * 0的话索引没变，1的话索引变为原索引加原来的数组长度。
